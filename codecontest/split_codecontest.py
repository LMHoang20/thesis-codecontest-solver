...
def augment(samples):
    ...
    in zip(samples["cf_contest_id"], samples["cf_index"], samples["solutions"], samples["incorrect_solutions"]):
        correct_prompt = [
            format(language, code, True) for language, code in zip(
                solutions["language"], solutions["solution"])
        ]

        incorrect_prompt = [
            format(language, code, False)
            for language, code in zip(incorrect_solutions["language"],
    ...
    return {"contest": contests, "index": indices, "prompt": prompts}
